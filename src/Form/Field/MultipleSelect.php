...
class MultipleSelect extends Select
{
    ...
    {
        $relations = \Illuminate\Support\Arr::get($data, $this->column);
        ...
        if (is_array($relations)) {
            if (is_null(current($relations))) {
                ...
                foreach ($relations as $relation) {
                    $this->value[] = \Illuminate\Support\Arr::get($relation, "pivot.{$this->getOtherKey()}");
                }
            }
        }
    }
    ...
    public function setOriginal($data)
    {
        ...
        if (is_array($relations)) {
            if (is_null(current($relations))) {
                ...
                foreach ($relations as $relation) {
                    $this->original[] = \Illuminate\Support\Arr::get($relation, "pivot.{$this->getOtherKey()}");
                }
            }
        }
    }
