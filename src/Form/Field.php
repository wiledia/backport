...
//        }
        ...
        $this->value = \Illuminate\Support\Arr::get($data, $this->column);
    ...
    public function setOriginal($data)
    {
        if (is_array($this->column)) {
            foreach ($this->column as $key => $column) {
                $this->original[$key] = \Illuminate\Support\Arr::get($data, $column);
            }

            return;
        }

        $this->original = \Illuminate\Support\Arr::get($data, $this->column);
    }
    ...
    public function getValidator(array $input)
    {
        ...
        if (is_string($this->column)) {
            if (!\Illuminate\Support\Arr::has($input, $this->column)) {
                return false;
            }
        ...
        if (is_array($this->column)) {
            foreach ($this->column as $key => $column) {
                ...
                $input[$column.$key] = \Illuminate\Support\Arr::get($input, $column);
    ...
    protected function sanitizeInput($input, $column)
    {
        if ($this instanceof Field\MultipleSelect) {
            $value = \Illuminate\Support\Arr::get($input, $column);
            \Illuminate\Support\Arr::set($input, $column, array_filter($value));
        }
