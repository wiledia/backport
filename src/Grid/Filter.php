...
class Filter implements Renderable
{
    ...
    {
        $inputs = \Illuminate\Support\Arr::dot(Request::all());
        ...
        foreach ($inputs as $key => $value) {
            \Illuminate\Support\Arr::set($params, $key, $value);
        }
        ...
        foreach ($this->filters() as $filter) {
            if (in_array($column = $filter->getColumn(), $this->layoutOnlyFilterColumns)) {
                $filter->default(\Illuminate\Support\Arr::get($params, $column));
            } else {
    ...
    protected function fullUrlWithoutQuery($keys)
    {
        ...
        $query = $request->query();
        \Illuminate\Support\Arr::forget($query, $keys);
