...
class Form implements Renderable
{
    ...
    protected function prepare($data = [])
    {
        ...
        $this->updates = \Illuminate\Support\Arr::except($this->inputs, array_keys($this->relations));
    }
    ...
    {
        \Illuminate\Support\Arr::forget($input, $this->ignored);
    ...
    public function update($id, $data = null)
    {
        ...
        if ($validationMessages = $this->validationMessages($data)) {
            if (!$isEditable) {
                ...
                return response()->json(['errors' => \Illuminate\Support\Arr::dot($validationMessages->getMessages())], 422);
            }
        }
    ...
    protected function handleEditable(array $input = [])
    {
        if (array_key_exists('_editable', $input)) {
            ...
            \Illuminate\Support\Arr::forget($input, ['pk', 'value', 'name']);
            \Illuminate\Support\Arr::set($input, $name, $value);
        }
            ...
            switch (true) {
                ...
                case $relation instanceof Relations\MorphMany:
                    ...
                    foreach ($prepared[$name] as $related) {
                        ...
                        $instance = $relation->findOrNew(\Illuminate\Support\Arr::get($related, $keyName));
                        ...
                        \Illuminate\Support\Arr::forget($related, static::REMOVE_FLAG_NAME);
    ...
    protected function prepareUpdate(array $updates, $oneToOneRelation = false)
    {
        ...
        foreach ($this->builder->fields() as $field) {
            ...
            if (!\Illuminate\Support\Arr::has($updates, $columns)) {
                continue;
            }
            ...
            if (is_array($columns)) {
                foreach ($columns as $name => $column) {
                    \Illuminate\Support\Arr::set($prepared, $column, $value[$name]);
                }
            } elseif (is_string($columns)) {
                \Illuminate\Support\Arr::set($prepared, $columns, $value);
            }
        }
    ...
    protected function prepareInsert($inserts)
    {
        if ($this->isHasOneRelation($inserts)) {
            $inserts = \Illuminate\Support\Arr::dot($inserts);
        }
        ...
        foreach ($inserts as $key => $value) {
            \Illuminate\Support\Arr::set($prepared, $key, $value);
        }
    ...
    protected function getDataByColumn($data, $columns)
    {
        if (is_string($columns)) {
            return \Illuminate\Support\Arr::get($data, $columns);
        }

        if (is_array($columns)) {
            $value = [];
            foreach ($columns as $name => $column) {
                if (!\Illuminate\Support\Arr::has($data, $column)) {
                    continue;
                }
                $value[$name] = \Illuminate\Support\Arr::get($data, $column);
            }
...
//        static::doNotSnakeAttributes($this->model);
    ...
    {
        if (is_null($value)) {
            return \Illuminate\Support\Arr::get($this->inputs, $key);
        }

        return \Illuminate\Support\Arr::set($this->inputs, $key, $value);
    }
    ...
    {
        \Illuminate\Support\Arr::forget(static::$availableFields, $abstract);
    }
    ...
    public static function findFieldClass($method)
    {
        ...
        $class = \Illuminate\Support\Arr::get(static::$availableFields, $method);
    ...
    public static function collectFieldAssets()
    {
        ...
        foreach (static::$availableFields as $field) {
            ...
            $css->push(\Illuminate\Support\Arr::get($assets, 'css'));
            $js->push(\Illuminate\Support\Arr::get($assets, 'js'));
        }
    ...
    {
        return \Illuminate\Support\Arr::set($this->inputs, $name, $value);
    }
    ...
    public function __call($method, $arguments)
    {
        if ($className = static::findFieldClass($method)) {
            $column = \Illuminate\Support\Arr::get($arguments, 0, ''); //[0];
